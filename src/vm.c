...
void interpret(size_t offset , bool panic) //by default panic if stack is not empty when finished
{
    ...
    CASE_CP CALLMETHOD:
    {
        ...
        else if (p1.type == 'z')
        {
            ...
            if (strcmp(method_name,"isalive") == 0)
            {
                ...
                zobject isAlive;
                isAlive.type = Z_BOOL;
                isAlive.i = (g->state != COROUTINE_STATE_STOPPED);
                zobject tmp;
            ...
            g->state = COROUTINE_STATE_RUNNING;
    ...
    CASE_CP SETUP_LOOP:
    {
        ...
        if(STACK.arr[lcv_idx].type == Z_INT)
            promote_numeric_type(&STACK.arr[lcv_idx],Z_INT64);
        if(STACK.arr[end_idx].type == Z_INT)
            promote_numeric_type(&STACK.arr[end_idx], Z_INT64);
        if(STACK.arr[end_idx+1].type == Z_INT)
            promote_numeric_type(&STACK.arr[end_idx+1], Z_INT64);
...
//        STACK.arr[STACK.size++] = p1;
    ...
    CASE_CP YIELD:
    {
        ...
        g->curr = ip - program + 1;
        g->state = COROUTINE_STATE_SUSPENDED;
        g->give_val_on_resume = false;
        frames.size--;
        zlist_push(&STACK,p1);
    ...
    CASE_CP ADD:
    {
        ...
        else if (is_numeric(p1.type) && is_numeric(p2.type))
        {
            ...
            c1 = Z_INT;
            promote_numeric_type(&p1, c1);
            promote_numeric_type(&p2, c1);
        }
        else
        { 
    ...
    CASE_CP EQ:
    {
        ...
        if (p1.type == Z_INT && p2.type == Z_INT64)
            promote_numeric_type(&p1, Z_INT64);
        else if (p1.type == Z_INT64 && p2.type == Z_INT)
            promote_numeric_type(&p2, Z_INT64);
    ...
    CASE_CP MUL:
    {
        ...
        else if (is_numeric(p1.type) && is_numeric(p2.type))
        {
            ...
            t = Z_INT;
            promote_numeric_type(&p1, t);
            promote_numeric_type(&p2, t);
        }
    ...
    CASE_CP MOD:
    {
        ...
        if (is_numeric(a.type) && is_numeric(b.type))
        {
            ...
            t = Z_INT;
            promote_numeric_type(&a, t);
            promote_numeric_type(&b, t);
        }
        else
        {
            orgk = ip - program;
...
zmodule* vm_alloc_zmodule(const char* name)
{
    zmodule* p = (zmodule*)malloc(sizeof(zmodule));
    if (!p)
    {
        fprintf(stderr,"alloc_zmodule(): error allocating memory!\n");
        exit(0);
    }
    p->name = name; 
    strmap_init(&(p->members));
...
zfun* vm_alloc_coro() //coroutine can be represented by zfun
{
    ...
    if (!p)
    {
        fprintf(stderr,"alloc_coroutine(): error allocating memory!\n");
        exit(0);
    }
    p->_klass = NULL;
    allocated += sizeof(zfun);
